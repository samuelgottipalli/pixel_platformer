...
class HUD:
    ...
    def _draw_health_bar(self, surface, player):
        ...
        # Background
        pygame.draw.rect(
            surface, (60, 0, 0), (x, y, health_width, health_height), border_radius=3
        )
        ...
        health_color = (
            ...
            else (YELLOW if health_percent > 0.25 else RED)
        )
        pygame.draw.rect(
            surface,
            health_color,
        ...
        # Border
        pygame.draw.rect(
            surface, UI_BORDER, (x, y, health_width, health_height), 1, border_radius=3
        )
    ...
    def _draw_level_info(self, surface, current_level, area_name, level_name):
        ...
        if level_name:
            level_text = self.font_large.render(
                f"ACT {act_num} - {level_name}", True, CYAN
            )
            surface.blit(
                level_text, (SCREEN_WIDTH // 2 - level_text.get_width() // 2, 8)
            )
        ...
        if area_name:
            ...
            surface.blit(
                area_text, (SCREEN_WIDTH // 2 - area_text.get_width() // 2, 32)
            )
    ...
    def draw_controls_overlay(self, surface):
        ...
        # Border
        pygame.draw.rect(
            surface, UI_BORDER, (overlay_x, overlay_y, 280, 260), 2, border_radius=5
        )
