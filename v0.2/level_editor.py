...
from config.settings import (
    ...
    BLACK, WHITE, GRAY, RED, GREEN, BLUE, YELLOW, CYAN, ORANGE, PURPLE
)
...
class LevelEditor:
    ...
    def _draw_tiles(self):
        ...
        for tile in self.tiles:
            ...
            if -self.grid_size < x < SCREEN_WIDTH and -self.grid_size < y < SCREEN_HEIGHT:
                ...
                pygame.draw.rect(self.screen, WHITE, (x, y, self.grid_size, self.grid_size), 1)
    ...
    def _draw_enemies(self):
        ...
        for enemy in self.enemies:
            ...
            if -self.grid_size < x < SCREEN_WIDTH and -self.grid_size < y < SCREEN_HEIGHT:
                ...
                pygame.draw.rect(self.screen, WHITE, (x, y, self.grid_size, self.grid_size), 2)
    ...
    def _draw_ui(self):
        ...
        # Sub-mode
        if self.mode == 'enemy':
            sub_text = self.font_small.render(f"Type: {self.selected_enemy_type} (E to cycle)", True, WHITE)
            self.screen.blit(sub_text, (10, 35))
        elif self.mode == 'hazard':
            sub_text = self.font_small.render(f"Type: {self.selected_hazard_type} (H to cycle)", True, WHITE)
            self.screen.blit(sub_text, (10, 35))
        elif self.mode == 'powerup':
            sub_text = self.font_small.render(f"Type: {self.selected_powerup_type} (P to cycle)", True, WHITE)
            self.screen.blit(sub_text, (10, 35))
            
        # Theme
        theme_text = self.font_small.render(f"Theme: {self.selected_theme} (T to change)", True, CYAN)
        self.screen.blit(theme_text, (10, 60))
        
        # Time limit
        time_text = self.font_small.render(f"Time Limit: {self.time_limit} (L to change)", True, CYAN)
